...
export const useSyncStore = create<SyncState>((set, get) => ({
  ...
  sync: () => {
    ...
    // Ohne Netz gibt es nichts abzugleichen. Das ist der einzige Weg in den
    // Zustand `error` (Blatt `4c`) und zugleich der ehrliche: einen Fehler
    // vorzufuehren, den es nicht gibt, waere dasselbe, wie den Abgleich
    // vorzutaeuschen, den es auch noch nicht gibt.
