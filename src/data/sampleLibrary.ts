...
function generated(): LibraryDocument[] {
  ...
  for (let i = curated.length; i < TOTAL; i += 1) {
    ...
    rest.push({
      ...
      openCount: (i * 11) % 19,
      // Ein nie geoeffnetes Dokument hat kein Datum — `null` ist hier eine
      // Aussage und kein fehlender Wert. Schrittweite 13 bei Periode 29: beide
      // prim, und 29 teilt keine der anderen Perioden (17, 19, 23, 24, 5, 7).
      // Sonst gehoerte zu jeder Ueberschrift genau ein "zuletzt geoeffnet".
      lastOpenedAt: (i * 11) % 19 === 0 ? null : now - ((n * 13) % 29) * DAY,
    });
  }
